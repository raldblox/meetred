...
export default function RoomPage({ roomId }: { roomId: string }) {
    ...
    >
      ...
      <span className="pointer-events-none absolute bottom-3 left-3">
        <Chip
          ...
          variant="dot"
        >
          <span className="text-xs font-semibold">You - {isHost ? 'Host' : isJoined ? 'Guest' : 'Offline'}</span>
        </Chip>
      </span>
      {!isCameraEnabled && (
  ...
  const renderRemoteTile = (extraClasses = '', isFullscreenView = false) => {
    ...
    if (!showRemoteStatus) {
      return (
        ...
        >
          <div className="text-center text-sm font-semibold uppercase tracking-[0.3em]">Waiting for peer</div>
        </div>
      )
    }
        ...
        <span className="pointer-events-none absolute bottom-3 left-3">
          ...
          >
            <span className="text-xs font-semibold">
              {peerPresent ? `Peer - ${peerRole === 'host' ? 'Host' : 'Guest'}` : 'Peer - Offline'}
            </span>
          </Chip>
        </span>
        ...
        {showRemoteStatus && (
          <div className="absolute top-3 right-3 flex flex-col items-end gap-2">
            <Chip
              ...
              variant="bordered"
            >
              {remoteVideoActive ? <Camera className="h-3 w-3" /> : <CameraOff className="h-3 w-3" />}
            </Chip>
            <Chip
              ...
              variant="bordered"
            >
              {remoteAudioActive ? <Mic className="h-3 w-3" /> : <MicOff className="h-3 w-3" />}
            </Chip>
          </div>
        )}
      </div>
  ...
  const renderScreenSharePanel = (isFullView = false) => {
    const activeVideoRef = isRemoteScreenSharing ? remoteScreenVideoRef : screenShareVideoRef
      ...
      >
        ...
        <span className="pointer-events-none absolute bottom-3 left-3">
          <Chip
            ...
            variant="dot"
          >
            <span className="text-xs font-semibold">{screenShareChipLabel}</span>
          </Chip>
        </span>
      </div>
    )
  ...
  const remoteTile = renderRemoteTile(showScreenPanel ? 'flex-1 h-auto min-h-0' : 'h-full')
  ...
  const unifiedLayout = (
    ...
    >
      {showScreenPanel && (
        <MotionDiv key="screen-share" layout className="min-h-[220px]" transition={springTransition}>
      ...
      {!isFullscreen && (
        <header className="flex w-full flex-wrap items-center justify-between gap-4 rounded-2xl border border-default-100 /80 px-4 py-3  backdrop-blur">
          <div className="flex flex-col gap-0">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-default-400">Room id</p>
            <div className="flex items-center gap-0">
              <h1 className="text-2xl font-semibold text-default-900 break-all">{roomId}</h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {isCalling && !isAwaitingAnswer && (
              <>
                <Chip className="font-mono" color="default" size="sm" variant="flat">
                  {formattedDuration}
                </Chip>
                {latency !== null && (
                  <Chip
      ...
      {isFullscreen && (
        <div className="pointer-events-none fixed inset-0 z-40 flex flex-col items-start justify-between p-4">
          <div className="pointer-events-auto">
            <Chip className="rounded-full bg-background/80 text-foreground" size="sm">
              {status}
            </Chip>
          </div>
        ...
        {isJoined && (
          <div className="flex gap-2">
            <Button
              ...
              radius="full"
              startContent={isCameraEnabled ? <Video size={16} /> : <VideoOff size={16} />}
          ...
          {() => (
            <div className="flex flex-col items-center gap-5">
              ...
              <div className="text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.4em] text-foreground/60">Incoming call</p>
                <h3 className="mt-1 text-2xl font-semibold">{incomingCaller ?? 'Guest'}</h3>
                <p className="text-sm text-foreground/60">wants to start a call with you</p>
              </div>
