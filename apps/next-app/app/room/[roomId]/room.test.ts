...
class MockMediaStream {
  ...
  constructor(
    ...
    if (opts.video) {
      this.tracks.push(new MockMediaStreamTrack('video', opts.videoLabel ?? 'video'))
    }
    if (opts.audio) {
      this.tracks.push(new MockMediaStreamTrack('audio', opts.audioLabel ?? 'audio'))
    }
  }
...
class MockRTCPeerConnection {
  ...
  removeTrack(sender: (typeof this.senders)[number]) {
    MockRTCPeerConnection.removedSenderLabels.push(sender.track?.label ?? sender.track?.kind ?? 'unknown')
...
class MockRealtimeChannel {
  ...
  getSignals() {
    return this.broadcasts.filter((item) => item.event === 'signal').map((item) => item.payload)
  }

  getRoomEvents() {
    return this.broadcasts.filter((item) => item.event === 'room-event').map((item) => item.payload)
  }
}
...
describe('useRoomController', () => {
  ...
  beforeEach(() => {
    ...
    getUserMediaMock = vi.fn(
      async (constraints?: MediaStreamConstraints) =>
        new MockMediaStream({
          video: typeof constraints?.video === 'boolean' ? constraints?.video : Boolean(constraints?.video),
          audio: typeof constraints?.audio === 'boolean' ? constraints?.audio : Boolean(constraints?.audio),
          videoLabel: 'camera',
  ...
  const getLatestMediaState = () => {
    ...
    const mediaEvents = activeChannel.getRoomEvents().filter((event) => event.type === 'media-state')
  ...
  it('joins the room and announces presence', async () => {
    ...
    expect(channel.getRoomEvents().some((evt) => evt.type === 'joined')).toBe(true)
  })
  ...
  it('starts a call and emits offer plus call-start events', async () => {
    ...
    expect(channel.getSignals().some((payload) => payload.type === 'offer')).toBe(true)
    expect(channel.getRoomEvents().some((evt) => evt.type === 'call-start')).toBe(true)
  })
  ...
  it('accepts an incoming offer and responds with an answer', async () => {
    ...
    expect(hook.result.current.isCalling).toBe(true)
    expect(channel.getSignals().some((payload) => payload.type === 'answer')).toBe(true)
  })
  ...
  it('toggles camera and microphone before a call and broadcasts media-state updates', async () => {
    ...
    expect(wasMediaRequested((constraints) => Boolean(constraints?.video))).toBe(true)
    ...
    expect(wasMediaRequested((constraints) => Boolean(constraints?.audio))).toBe(true)
    expect(getLatestMediaState()).toMatchObject({
      micEnabled: true,
    })
  ...
  it('renegotiates media when toggling devices during an active call', async () => {
    ...
    const initialOfferCount = channel.getSignals().filter((payload) => payload.type === 'offer').length
    ...
    const renegotiatedOfferCount = channel.getSignals().filter((payload) => payload.type === 'offer').length
  ...
  it('recovers when the remote peer disconnects and rejoins mid-call', async () => {
    ...
    const joinedAckCount = roomEvents.filter((evt) => evt.type === 'joined-ack').length

    expect(joinedAckCount).toBeGreaterThanOrEqual(1)
    expect(roomEvents.some((evt) => evt.type === 'call-start')).toBe(true)
    const totalOffers = channel.getSignals().filter((payload) => payload.type === 'offer').length
  ...
  describe('Priority scenarios', () => {
    ...
    it('handles channel status errors', async () => {
      ...
      await waitFor(() => expect(hook.result.current.status).toBe('Channel timed out'))

      await act(async () => {
        activeChannel!.triggerStatus('CHANNEL_ERROR')
      })
      await waitFor(() => expect(hook.result.current.status).toBe('Channel error'))

      await act(async () => {
        activeChannel!.triggerStatus('CLOSED')
      })
      await waitFor(() => expect(hook.result.current.status).toBe('Channel closed'))
    })
    ...
    it('enforces room capacity by emitting room-full events', async () => {
      ...
      expect(channel.getRoomEvents().some((evt) => evt.type === 'room-full' && evt.target === 'peer-2')).toBe(true)
    })
    ...
    it('keeps screen sharing active when toggling camera on the host', async () => {
      ...
      expect(MockRTCPeerConnection.removedSenderLabels.filter((label) => label.includes('screen')).length).toBe(0)
    })
    ...
    it('keeps the remote camera stream visible when a peer starts screen share', async () => {
      ...
      hook.result.current.remoteVideoRef.current = document.createElement('video')
      hook.result.current.remoteScreenVideoRef.current = document.createElement('video')
      ...
      await waitFor(() => expect(hook.result.current.remoteVideoActive).toBe(true))
      ...
      await act(async () => {
        pc!.ontrack?.({
          track: remoteScreenStream.getVideoTracks()[0] as unknown as MediaStreamTrack,
          streams: [remoteScreenStream as unknown as MediaStream],
        } as RTCTrackEvent)
        await flushAsync()
      })
      await waitFor(() => expect(hook.result.current.isRemoteScreenSharing).toBe(true))
